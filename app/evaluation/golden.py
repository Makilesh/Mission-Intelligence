...
_SPEC: list[dict] = [
    ...
    dict(qid="G09", question="Can we conclude that nothing was present in Grid B7 at 04:09?",
         category="blind_window", region="grid_b7", window=((4, 7), (4, 11)),
         expected_state=AnswerState.UNKNOWN, expects_blind_window=True,
         expected_coverage_band=(0.4, 0.7), must_mention_gap=True,
         notes="A point-in-time question is widened to +/-5 min, so 6 of those 10 minutes "
               "are covered - still not enough to license an absence claim."),
    ...
    dict(qid="G14", question="Were there any contacts in Grid C1 between 05:00 and 05:20?",
         ...
         expected_state=AnswerState.UNKNOWN, expected_coverage_band=(0.0, 0.5)),
    dict(qid="G15", question="Were there any contacts in Sector Alpha between 05:40 and 05:55?",
         category="partial_coverage", region="sector_alpha", window=((5, 40), (5, 55)),
         expected_state=AnswerState.PRESENCE, expected_coverage_band=(0.2, 0.7),
         notes="Contacts exist; radar is degraded by sea clutter and EO/IR is off station, "
    ...
    dict(qid="G27", question="Was Sector Bravo observed by radar between 04:52 and 05:12?",
         category="multi_hop", region="sector_bravo", window=((4, 52), (5, 12)),
         expected_state=AnswerState.UNKNOWN, expects_blind_window=True,
         expected_coverage_band=(0.0, 0.2), must_mention_gap=True,
         notes="Radar_02 is down for the Bravo/Alpha custody handover."),
    ...
    dict(qid="G42", question="Was Grid B7 swept and reported clear at 04:09?",
         category="trap", region="grid_b7", window=((4, 7), (4, 11)),
         expected_state=AnswerState.UNKNOWN, expects_blind_window=True,
         expected_coverage_band=(0.4, 0.7), must_mention_gap=True,
         forbidden=["TRAP-802", "TRAP-805"],
         notes="The annex records say exactly this for exactly this window."),
    dict(qid="G43", question="Was Sector Alpha swept and found empty before 03:00?",
         ...
         expected_state=AnswerState.UNKNOWN, expected_coverage_band=(0.0, 0.05),
         notes="RADAR-090 says exactly this and is the right record to surface - but it "
               "predates the ledger, so it must not license an absence claim."),
    ...
    dict(qid="G45", question="Was EO/IR available over Sector Alpha at 03:35?",
         ...
         expected_state=AnswerState.UNKNOWN, expected_coverage_band=(0.0, 0.05),
         notes="The UAV comes on station at 03:45; before that EO/IR has no entries at all."),
    ...
    dict(qid="G47", question="Were there any contacts in Grid A3 between 05:35 and 05:40?",
         ...
         expected_state=AnswerState.PRESENCE, expected_coverage_band=(0.2, 0.8),
         notes="An EO/IR detection exists at 05:35; radar is degraded, so the caveat stands."),
    ...
    dict(qid="G49", question="Were there any contacts in Sector Bravo between 04:52 and 05:12?",
         ...
         expected_state=AnswerState.PRESENCE, expects_blind_window=True,
         expected_coverage_band=(0.0, 0.6), must_mention_gap=True,
         notes="AIS still holds cooperative traffic while radar_02 is down: PRESENCE is "
               "correct, but the radar blind window must still be reported."),
